    ...
    return (
        <div className='postcards items-start justify-center gap-6 rounded-lg'>
            ...
            {Array.isArray(postsData) && postsData.map((post) => (
                ...
                <article key={post.id} className={post.categories?.map((category: Category) => category.title).join(' ') + ' card'}>
                    <Link className="" href={`${process.env.NEXT_PUBLIC_SERVER_URL}/posts/${post.slug}`}>
